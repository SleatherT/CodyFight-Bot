...
# IMPORTANT: To work as except the order must be a unique number for each ability you are going to use

# This Flag/Boolean makes the bots prioritize spending energy in killing if a close enemy agent is low life
prioKill_flag = True
prioKill_order = 1
prioKill_name = "prioKill"

# This flag makes the bot target the enemy with less life if there is two or more objectives 
smartTargeting_flag = True
smartTargeting_order = 2
...
# that uses the order of the flag as key and the dict as value, this is made to sort the modules and execute as planned)
...
# This variable is used to define which skill to prioritize, that means the bot will always save energy for this skill and use it first,if you 
# dont want to prioritized any skill put a random number 
